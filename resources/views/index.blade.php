    ...
    <div class="flex flex-wrap">
            ...
            @if (count($posts) > 0)
                @foreach ($posts as $post)
                    <x-collection id="{{ $post->id }}" mimetype="{{ $post->mimetype }}" material="{{ $post->material }}"></x-collection>
                @endforeach
            @endif
...
<div class="bg-white">
  ...
  <nav class="sticky bottom-0 mx-auto flex w-full justify-between gap-8 border-t bg-white px-10 py-4 text-xs sm:max-w-md sm:rounded-t-xl sm:border-transparent sm:text-sm sm:shadow-2xl">
    ...
    <a href={{ url('/dashboard') }} class="flex flex-col items-center gap-1 text-gray-400 transition duration-100 hover:text-gray-500 active:text-gray-600">
      <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
